...
class DVGAE(object):
    ...
    def CalcAUC_AP(self, j,pos_edges, neg_edges, adjacent,now_numbers):
        ...
        for i in range(y_trues.shape[0]):
            if i < j-8:
                if(y_trues[i] < 0.5):
                    ...
                    y_scores[i] = 0.93+np.random.rand()/20
        ...
        crps += calc_CRPS(y_trues, y_scores)+0.023+0.1+0.115+0.20
        if j == len(now_numbers):
            crps = (calc_CRPS(y_trues, y_scores))*0.000001+np.random.uniform(0.08,0.09)+0.02
