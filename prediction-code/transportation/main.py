...
parser.add_argument('--batch_size', type=int, default=8, help='Batch Size during training [default: Tdrive: 8, los_loop: 16, PEMS08: 16]')
...
if __name__ == "__main__":
    ...
    all_data = read_and_generate_dataset(graph_signal_matrix_filename, generated_adj, num_of_weeks,  num_of_days, num_of_hours,   num_for_predict, num,points_per_hour,
                                         merge)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for train_w, train_d, train_r, train_t, train_adj_r in train_loader:
            ...
            # train_w = train_w + noise_level * torch.randn_like(train_w)
            # train_d = train_d + noise_level * torch.randn_like(train_d)
            # train_r = train_r + noise_level * torch.randn_like(train_r)
            # # 如果数据必须非负，下面这步可保留
            # train_w = torch.clamp(train_w, min=0)
            # train_d = torch.clamp(train_d, min=0)
            # train_r = torch.clamp(train_r, min=0)
            # # ======= 加噪声结束 =======
            #
            # ======= 加入随机mask模拟缺失（设为NaN） =======
            mask_percentage = 0.2  # mask掉20%的数据，可调整
            mask_w = torch.rand_like(train_w) < mask_percentage
            mask_d = torch.rand_like(train_d) < mask_percentage
            mask_r = torch.rand_like(train_r) < mask_percentage

            # 将被mask掉的位置设置为NaN，模拟真实缺失
            train_w_masked = train_w.clone()
            train_d_masked = train_d.clone()
            train_r_masked = train_r.clone()
        ...
        end_time1 = time()
        vald_time = end_time1-start_time1+vald_time
    ...
    print("all_train_time",np.sum(train_time))
    print("all_val_time",vald_time)
